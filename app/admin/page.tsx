...
function SectionCadastro() {
  // Adicionámos o 'collections' aqui para puxar as coleções existentes para o autocomplete
  ...
  useEffect(() => {
    if (editingProduct) {
      setForm({
        ...
        collection: editingProduct.collection?.toUpperCase() || "", // Garante maiúsculas ao editar
  ...
  const handleSave = async () => {
    if (!form.name || !form.sku || !form.rentalPrice) return // Bloqueio extra de segurança
    try {
      if (editingProduct) {
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          <div className="bg-white rounded-xl border border-border p-6 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {/* Autocomplete e forçando Maiúsculas no onChange */}
                <Input 
                  list="colecoes-list"
          ...
          <div className="bg-white rounded-xl border border-border p-6 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div>
                {/* Adicionado o asterisco e regra de obrigatoriedade */}
          ...
          <div className="flex items-center gap-3">
            {/* O botão agora exige rentalPrice (valor de aluguel) preenchido */}
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      if (res.ok) {
        ...
        addCollection({ id: data.id, ...form, active: false, createdAt: new Date().toISOString() })
      }
    }
    setIsCreating(false)
    setEditing(null)
            ...
            <div key={col.id} className={`bg-white rounded-xl border overflow-hidden transition-all ${col.active ? "border-primary/40 shadow-sm" : "border-border"}`}>
              <div className="px-5 py-4 flex items-center justify-between gap-3 flex-wrap border-b border-border">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openEdit(col)}><Pencil size={12} /></Button>
...
export default function AdminPage() {
  const { section, setOrders, setProducts, setStoreConfig, setCategories, setCollections } = useAdminStore()
  ...
  useEffect(() => {
    ...
    carregarDadosReais()
  }, [setOrders, setProducts, setStoreConfig, setCategories, setCollections])
