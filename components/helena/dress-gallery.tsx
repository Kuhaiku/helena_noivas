...
export function DressGallery({ dress, open, onClose }: DressGalleryProps) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "bg-background rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden transition-all duration-300 flex flex-col md:flex-row max-h-[90vh]",
          open ? "scale-100 opacity-100" : "scale-95 opacity-0"
        )}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Main image */}
        <div className="relative flex-1 min-h-[320px] md:min-h-[500px] bg-secondary shrink-0">
          <Image
        ...
        {/* Info panel */}
        <div className="w-full md:w-72 flex flex-col p-6 gap-5 justify-between overflow-y-auto">
            ...
            <div className="flex flex-col gap-2 text-sm">
              ...
              {dress.showPrice && (
                <div className="flex items-center justify-between py-2 border-b border-border">
                  ...
                  <span className="font-medium text-foreground">R$ {(dress.rentalPrice || 0).toLocaleString('pt-BR')}</span>
                </div>
              )}
          ...
          <button
            onClick={toggle}
            className={cn(
              "w-full flex items-center justify-center gap-2 py-3 rounded-xl font-sans font-semibold text-sm tracking-wide transition-all duration-200 active:scale-[0.98] mt-4",
              inCart
